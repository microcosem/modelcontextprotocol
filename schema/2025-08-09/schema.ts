/* JSON-RPC types (Security-focused MCP schema) */
...
/** @internal */
export const LATEST_PROTOCOL_VERSION = "2025-08-09";
/** @internal */
...
export interface Request {
  ...
  params?: {
    ...
    _meta?: {
      ...
      /**
       * Security envelope for this request. When present, receivers SHOULD validate according to their
       * security policy before processing the request.
       */
...
// MCP security-related error codes (non-standard JSON-RPC codes)
/** Unauthorized: missing or invalid identity/capabilities */
export const UNAUTHORIZED = 4900;
/** Forbidden: capability present but does not allow this action */
export const FORBIDDEN = 4901;
/** Replay detected based on nonce/timestamp */
export const REPLAY_DETECTED = 4902;
/** Invalid signature or integrity hash */
export const INVALID_SIGNATURE = 4903;
/** Capability token malformed */
export const INVALID_CAPABILITY = 4904;
/** Capability token expired or not yet valid */
export const EXPIRED_CAPABILITY = 4905;
/** Encryption required but not provided */
export const ENCRYPTION_REQUIRED = 4906;
/** Decryption failed (e.g., wrong key) */
export const DECRYPTION_FAILED = 4907;
...
export interface SecurityCapabilities {
   ...
   */
  acceptCapabilityFormats?: ("jwt" | "cwt" | "paseto" | "object")[];
  /**
...
export interface SecurityMeta {
  ...
  /**
   * Capability tokens authorizing this specific request. Each entry may be an opaque string (e.g., JWT)
   * or a structured object for constrained environments.
   */
  ...
  /**
   * Optional message-level encryption container. When present, `params` MAY be omitted
   * and the receiver is expected to decrypt to recover the plaintext parameters.
   */
...
export interface ResourceScope {
  type: "resource";
  actions: ("read" | "subscribe" | "unsubscribe" | "list" | "templates_list")[];
...
/**
 * Request capabilities from the server. Servers SHOULD validate the caller's identity proof and policy
 * before issuing any capabilities. Returned tokens are typically short-lived and fine-grained.
 *
 * @category security/issue
 */
export interface IssueCapabilitiesRequest extends Request {
...
export interface IssueCapabilitiesResult extends Result {
